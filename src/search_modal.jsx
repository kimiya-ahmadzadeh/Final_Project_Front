...
import { GetGenres, GetLangs, Search } from "./fetch_data";
...
export function SearchModal(props) {
    ...
    const searchBook = async () => {
        ...
        if (title != "") {
            books = books.filter((b) => b.title.includes(title));
        }
        if (author != "") {
            books = books.filter((b) => b.author.includes(author));
        }
        if (langs.length > 0) {
            let filterLang = [];
            books.forEach((b) => {
        ...
        books = books.filter((b) => b.year >= fromYear && b.year <= toYear);
        console.log(books);
    }
